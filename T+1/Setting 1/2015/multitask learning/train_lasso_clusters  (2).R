...
mse = numeric()
#coefficients for each cluster in each cluster number
coeff = list()
#uesrs in each cluster for each number of clustering
users = list()
for(i in 1:23){
  users[[i]] = lapply(1:i,function(j) which(clusterings[i,] %in% j)) 
}
#lambdas 
lambdas = list()
...
for(i in 1:23){
  ...
  #Fit a lasso model for each cluster
  cv.fits = lapply(1:i, function(j) cv.glmnet(x = as.matrix(cur_df[[j]][,-1]), y = cur_df[[j]][,'y']))
  
  coeff[[i]] = t(sapply(1:i, function(j) as.vector(coef(cv.fits[[j]], s = 'lambda.min', alpha = 0))))
  lambdas[[i]] = sapply(1:i, function(j) cv.fits[[j]]$lambda.min)
  y_length = 0
  sse = 0
  for(j in 1:i) {
    print(j)
    y_hat = predict(cv.fits[[j]], newx = as.matrix(cur_df[[j]][,-1]), s = 'lambda.min')
    sse = sse + sum((cur_df[[j]][,1] - y_hat)^2)
    y_length = y_length + length(cur_df[[j]][,1])
  }
  mse[i] = sse/y_length
}
plot(1:23, mse,type = 'b', xlab = 'number of clusters', ylab = 'test mse')
col1 = rep(1:23, time = 1:23)
coeff_df_big <- ldply(coeff, data.frame)
lambdas_big <- unlist(lambdas)
final_df = cbind(col1,lambdas_big,coeff_df_big)
final_df
colnames(final_df) = c('cluster number', 'lambdas',colnames(m1))
write.csv(final_df, file = 'coeff_clustering.csv')
write.csv(mse,file = 'mse_train.csv')
