...
full_2015_data$UserID = full_2015_data$userid
...
impute_df = function(cur_user) {
  ...
  cur_data = cur_data[, c(mood_var,sleep_vars,dist_vars)]
  #imputation
  cur_data[,mood_var] = impute_mood(cur_data[,mood_var])
  for(i in c(sleep_vars,dist_vars)){
...
lasso_df = function(cur_data){
  ...
  cur_ts = x_new
  colnames(cur_ts) = c('mood', sleep_vars, dist_vars)
...
save(err_df, file = 'err_df_2015.rda')
